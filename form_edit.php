<?php
include 'koneksi.php';
$data_edit = mysqli_query($conn, "SELECT * FROM daftar_mengemudi WHERE id = '" . $_GET['id'] . "'");
...
<body>
                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    <ul class="navbar-nav ms-auto mb-2 mb-lg-0">
                        <li class="nav-item mx-3">
                            <a class="nav-link active" aria-current="page" href="index.php">Daftar Kursus</a>
                        </li>
                        <li class="nav-item dropdown mx-3">
                            ...
                            <ul class="dropdown-menu dropdown-menu-dark" aria-labelledby="navbarDropdown" style="background-color: #1A2238;">
                                <li><a class="dropdown-item text-white" href=".php">Kursus Mengemudi</a></li>
                                <li><a class="dropdown-item text-white" href=".php">Kursus Komputer</a></li>
                                <li><a class="dropdown-item text-white" href="bukti_bayar_digital_market.php">Kursus Digital Marketing</a></li>
                                <li><a class="dropdown-item text-white" href=".php">Kursus Bahasa Inggris</a></li>
                            </ul>
                        </li>
                        <li class="nav-item mx-3">
                            <a class="nav-link active" href="login_admin.php">Login Admin</a>
                        </li>
                    </ul>
                </div>
            </div>
                ...
                <form action="" method="post">
                    ...
                    <div class="input-group mb-3">
                        ...
                        <select class="form-select" aria-label="Default select example" name="paket" id="paket">
                            <option value=""><?php echo $result['paket'] ?></option>
                            <option value="Rp. 600.000,-">PAKET MANUAL 1 (Melancarkan)</option>
                            <option value="Rp. 900.000,-">PAKET MANUAL 2 (Pemula)</option>
                            <option value="Rp. 1.050.000,-">PAKET MANUAL 3 (Mahir)</option>
                            <option value="Rp. 650.000,-">PAKET MATIC 1 (Pemula)</option>
                            <option value="Rp. 950.000,-">PAKET MATIC 2 (Mahir)</option>
                        </select>
                        <input type="hidden" name="paket" id="namapaket" value="<?php echo $result['paket'] ?>">
                    </div>
                    <div class="input-group mb-3">
                        <span class="input-group-text" style="width: 135px; justify-content:center;">
                            <center>Harga</center>
                        </span>
                        <input type="text" class="form-control" name="harga" id="pilihpaket" value="<?php echo $result['harga'] ?>" readonly>
                    </div>
                    <div class="input-group mb-3">
                        <span class="input-group-text" style="width: 135px; justify-content:center;">
    ...
    <?php
    if (isset($_POST['edit'])) {
        $update = mysqli_query($conn, "UPDATE daftar_mengemudi SET nik = '" . $_POST['nik'] . "', nama = '" . $_POST['nama'] . "', alamat = '" . $_POST['alamat'] . "', tempat_lahir = '" . $_POST['tempat_lahir'] . "', tanggal_lahir = '" . $_POST['tanggal_lahir'] . "', jk = '" . $_POST['jk'] . "', nama_ayah = '" . $_POST['nama_ayah'] . "', nama_ibu = '" . $_POST['nama_ibu'] . "', telepon = '" . $_POST['telepon'] . "', email = '" . $_POST['email'] . "', kecamatan = '" . $_POST['kecamatan'] . "', paket = '" . $_POST['paket'] . "', harga = '" . $_POST['harga'] . "', pilih_bayar = '" . $_POST['pilih_bayar'] . "' WHERE id = '" . $_GET['id'] . "' ");
    }
    if ($update) {
        echo "
                <script>
                    alert('Data Telah Di UPDATE!');window.location='data.php'
                </script>
                ";
    }
    ?>
