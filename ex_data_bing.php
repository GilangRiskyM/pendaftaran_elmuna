...
$query = "SELECT * FROM daftar_bahasa_inggris ORDER BY tanggal_upload ASC";
...
if (isset($_GET["export"])) {
  if (empty($_GET["tglawal"])) {
    ...
    $orderQuery = "SELECT * FROM daftar_bahasa_inggris WHERE tanggal_upload >= '" . $_GET["tglawal"] . "' AND tanggal_upload <= '" . $_GET["tglakhir"] . "' ORDER BY id ASC";
    ...
    if (count($filterOrders)) {
      ...
      $filename = "Laporan Kursus Bahasa Inggris " . $tglawal . " Sampai " . $tglakhir . ".xls";
      ...
      $header = "SELECT * FROM daftar_bahasa_inggris";
