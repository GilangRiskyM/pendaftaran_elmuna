...
$query = "SELECT * FROM daftar_mengemudi ORDER BY tanggal_upload ASC";
...
if (isset($_GET["export"])) {
  if (empty($_GET["tglawal"])) {
    ...
    $orderQuery = "SELECT * FROM daftar_mengemudi WHERE tanggal_upload >= '" . $_GET["tglawal"] . "' AND tanggal_upload <= '" . $_GET["tglakhir"] . "' ORDER BY id ASC";
    ...
    if (count($filterOrders)) {
      ...
      $filename = "Laporan Kursus Mengemudi " . $tglawal . " Sampai " . $tglakhir . ".xls";
      ...
      $header = "SELECT * FROM daftar_mengemudi";
